...
for i = 1 : length(sim_t) 
    ...
    d_qp(i) = (deriv2_qp>=0)*params.d_max(2) +...
              (deriv2_qp<0)* params.d_min(2);
    max_D = d_qp(i)*deriv2_qp;
    ...
    d_opt(i) = (deriv2_opt>=0)*params.d_max(2) +...
    ...
    d_sontag(i) = (deriv2_sontag>=0)*params.d_max(2) +...
                  (deriv2_sontag<0)* params.d_min(2);
    u_sontag(i) = sontag(LfV_sontag,LgV_sontag,d_sontag(i),params);
    [~, xs_sontag] = ode45(@(t, s) ...
...
figure
set(gcf,'unit','normalized'.*ones(151,151),'position',[0.1,0.25,0.8,0.55]);
subplot(1,2,1)
plot(x0(1),x0(2),'r*')
hold on
...
legend('IC point','qp','opt','sontag','interpreter','latex','FontSize',18)
...
dydt = [s(2);(-s(2)+sin(s(1)))+u+d];
end

function u = sontag(LfV,LgV,de,params)
    umin = params.u_min;
    umax = params.u_max;
    u_size = size(LgV,2);
    u = nan(u_size,1);
